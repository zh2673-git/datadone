...
class WeChatAnalyzer(BaseAnalyzer):
    ...
    def analyze(self, analysis_type: str = 'all', source_name: Optional[str] = None) -> Dict[str, pd.DataFrame]:
        ...
        Dict[str, pd.DataFrame]
            分析结果, 键为结果名 (例如 '吴平一家明细.xlsx_微信频率'), 值为结果数据
        """
    ...
    def _analyze_for_source(self, source_name: str, analysis_type: str) -> Dict[str, pd.DataFrame]:
        ...
        """
        results = {}
        source_data = self.wechat_model.data[self.wechat_model.data['数据来源'] == source_name]
        ...
        if analysis_type in ['frequency', 'all']:
            ...
            if not frequency_result.empty:
                results[f"{source_name}_微信频率"] = frequency_result
    ...
    def analyze_frequency(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        if data.empty:
            self.logger.warning("没有微信交易记录数据")
            return pd.DataFrame()

        # 获取相关列名
        name_col = self.wechat_model.name_column
        opposite_name_col = self.wechat_model.opposite_name_column
        date_col = self.wechat_model.date_column
