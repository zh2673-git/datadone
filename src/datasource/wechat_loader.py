...
class WechatLoader(BaseLoader):
    ...
    def load(self, path: str) -> dict[str, pd.DataFrame]:
        ...
        # 2. 字段映射
        df = self.field_mapper.map_fields(df, 'wechat')

        # 3. 借贷标识标准化：微信原始标识为'入'/'出'，保持原值
        # (收入/支出金额计算移到分析层)
        ...
        # 8. 添加默认列（微信不执行存取现识别）
        df = self._add_default_columns(df, 'wechat')
