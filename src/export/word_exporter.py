...
class WordExporter:
    ...
    def generate_person_bank_analysis(self, doc: Document, person_name: str, analyzer):
        ...
        if not transfer_freq_df.empty:
            self._generate_frequency_summary_paragraph(doc, person_name, transfer_freq_df, person_data, "银行转账", analyzer.bank_model)
        ...
        if not cash_ops_df.empty:
            ...
            if not deposit_df.empty:
                 self._generate_cash_summary_paragraph(doc, person_name, deposit_df, '存现', analyzer.bank_model)
            
            withdraw_df = cash_ops_df[cash_ops_df['存取现标识'] == '取现']
            if not withdraw_df.empty:
                 self._generate_cash_summary_paragraph(doc, person_name, withdraw_df, '取现', analyzer.bank_model)
    ...
    def generate_person_payment_analysis(self, doc: Document, person_name: str, analyzer, payment_type: str):
        ...
        if not freq_df.empty:
            self._generate_frequency_summary_paragraph(doc, person_name, freq_df, person_data, f"{payment_type}交易", analyzer.data_model)
    ...
    def _generate_frequency_summary_paragraph(self, doc: Document, person_name: str, frequency_df: pd.DataFrame, person_data: pd.DataFrame, analysis_type_str: str, data_model):
        ...
        # 如果重复最多的金额大于5000元，加粗显示
        p = doc.add_paragraph()
        if abs(most_frequent_amount) > 5000:
            p.add_run('重复最多的金额为').bold = False
            p.add_run(f"{most_frequent_amount:,.2f}元").bold = True
    ...
    def _generate_cash_summary_paragraph(self, doc: Document, person_name: str, cash_df: pd.DataFrame, cash_type: str, bank_model):
        """生成存取现分析的概要段落"""
        if cash_df.empty:
            return
        ...
        start_date = cash_df['最早交易日'].min()
        end_date = cash_df['最晚交易日'].max()
        time_span_str = f"{start_date.strftime('%Y-%m-%d')}至{end_date.strftime('%Y-%m-%d')}"

        # 概览段落
        p = doc.add_paragraph()
        if cash_type == '存现':
            p.add_run("2、存现概览").bold = True
        else:
            p.add_run("3、取现概览").bold = True
        
        p.add_run(f"：在 {time_span_str} 期间，累计{cash_type} {total_count} 次，总金额为 ")
        
        # 处理存现总金额的格式
        if cash_type == '存现':
            amount_run = p.add_run(f"{total_amount:,.2f} 元")
            amount_run.bold = True
            if total_amount > 1000000:  # 超过100万
                amount_run.underline = True
        else:
            p.add_run(f"{total_amount:,.2f} 元")
        
        p.add_run("。")

        # 获取原始数据进行详细分析
        raw_data = bank_model.data[
        ...
        if not raw_data.empty:
            # 计算主要时间集中
            raw_data['年份月份'] = raw_data[bank_model.date_column].dt.strftime('%Y年%m月')
            monthly_counts = raw_data.groupby('年份月份').size().reset_index(name='次数')
            top_months = monthly_counts.nlargest(3, '次数')
            major_time_str = ", ".join([f"{row['年份月份']} ({row['次数']}次)" for _, row in top_months.iterrows()])
            doc.add_paragraph(f"主要时间集中在：{major_time_str}。")
            ...
            all_amounts = raw_data[amount_col].abs().sort_values(ascending=False)
            top_single_amounts = ", ".join([f"{x:,.2f}元" for x in all_amounts.head(3).tolist()])
            doc.add_paragraph(f"单笔主要金额：{top_single_amounts}。")

            # 重复最多的金额
            amount_counts = raw_data[amount_col].value_counts()
            if not amount_counts.empty:
                ...
                # 如果是取现且金额大于5000，加粗显示
                p = doc.add_paragraph()
                if cash_type == '取现' and abs(most_frequent_amount) > 5000:
                    p.add_run('重复最多的金额为').bold = False
                    p.add_run(f"{most_frequent_amount:,.2f}元").bold = True
                    p.add_run(f" ({most_frequent_count}次)。").bold = False
                else:
                    p.add_run(f"重复最多的金额为{most_frequent_amount:,.2f}元 ({most_frequent_count}次)。")

            # 排名前五的单笔金额
            if cash_type == '存现':
                doc.add_paragraph("排名前五的单笔存现金额如下：")
                top_5_deposits = raw_data.nlargest(5, amount_col)
                
                # 定义要显示的列
                display_cols = [bank_model.date_column]  # 交易日期列一定存在
                display_cols.append(amount_col)  # 金额列一定存在
                
                # 检查可选列是否存在
                optional_cols = ['摘要', '备注', '交易摘要', '备注信息']
                for col in optional_cols:
                    if col in top_5_deposits.columns:
                        display_cols.append(col)
                
                # 重命名列
                rename_dict = {
                    bank_model.date_column: '交易日期',
                    amount_col: f'{cash_type}金额'
                }
                ...
                display_df = top_5_deposits[display_cols].rename(columns=rename_dict)
