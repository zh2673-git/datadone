...
class KeyTransactionEngine:
    ...
    def identify_key_transactions(self, data: pd.DataFrame, summary_column: str, 
        ...
        result_data['是否房产支出'] = False
        result_data['是否租金收入'] = False
        result_data['是否租金支出'] = False
        result_data['是否车辆收入'] = False
        result_data['是否车辆支出'] = False
        result_data['是否证券收入'] = False
        result_data['是否证券支出'] = False
        ...
        # 添加摘要列（如果存在）
        if summary_column and summary_column in result_data.columns:
            text_parts.append(result_data[summary_column].fillna('').astype(str))

        # 添加备注列（如果存在）
        if remark_column and remark_column in result_data.columns:
            text_parts.append(result_data[remark_column].fillna('').astype(str))

        # 添加类型列（如果存在）
        if type_column and type_column in result_data.columns:
            text_parts.append(result_data[type_column].fillna('').astype(str))

        # 添加对方姓名列（如果存在）
        if opposite_name_column and opposite_name_column in result_data.columns:
            text_parts.append(result_data[opposite_name_column].fillna('').astype(str))

        # 合并所有文本部分
        if text_parts:
            result_data['匹配文本'] = text_parts[0]
            for part in text_parts[1:]:
                result_data['匹配文本'] = result_data['匹配文本'] + ' ' + part
        else:
            result_data['匹配文本'] = ''
        ...
        # 识别车辆支出
        self._identify_vehicle_expense(result_data, amount_column)

        # 识别房产支出
        self._identify_property_expense(result_data, amount_column)

        # 识别证券支出
        self._identify_securities_expense(result_data, amount_column)

        # 识别租金支出
        self._identify_rental_expense(result_data, amount_column)
    ...
    def _identify_vehicle_expense(self, data: pd.DataFrame, amount_column: str):
        """识别车辆支出（买车等）"""
        # 只识别支出（负金额）
        ...
        if self.vehicle_keywords:
            vehicle_mask = data['匹配文本'].str.contains('|'.join(self.vehicle_keywords), case=False, na=False)
            final_vehicle_mask = expense_mask & vehicle_mask
            data.loc[final_vehicle_mask, '是否车辆支出'] = True
            data.loc[final_vehicle_mask, '是否重点收支'] = True

        vehicle_expense_count = data['是否车辆支出'].sum()
        self.logger.debug(f"识别出车辆支出交易 {vehicle_expense_count} 笔")

    def _identify_property_expense(self, data: pd.DataFrame, amount_column: str):
        """识别房产支出（买房等）"""
        # 只识别支出（负金额）
        expense_mask = data[amount_column] < 0
        ...
        if self.property_keywords:
            property_mask = data['匹配文本'].str.contains('|'.join(self.property_keywords), case=False, na=False)
            final_property_mask = expense_mask & property_mask
            data.loc[final_property_mask, '是否房产支出'] = True
            data.loc[final_property_mask, '是否重点收支'] = True

        property_expense_count = data['是否房产支出'].sum()
        self.logger.debug(f"识别出房产支出交易 {property_expense_count} 笔")

    def _identify_securities_expense(self, data: pd.DataFrame, amount_column: str):
        """识别证券支出（买股票等）"""
        # 只识别支出（负金额）
        expense_mask = data[amount_column] < 0
        ...
        if self.securities_keywords:
            securities_mask = data['匹配文本'].str.contains('|'.join(self.securities_keywords), case=False, na=False)
            final_securities_mask = expense_mask & securities_mask
            data.loc[final_securities_mask, '是否证券支出'] = True
            data.loc[final_securities_mask, '是否重点收支'] = True

        securities_expense_count = data['是否证券支出'].sum()
        self.logger.debug(f"识别出证券支出交易 {securities_expense_count} 笔")

    def _identify_rental_expense(self, data: pd.DataFrame, amount_column: str):
        """识别租金支出（租房等）"""
        # 只识别支出（负金额）
        expense_mask = data[amount_column] < 0
        ...
        if self.rental_keywords:
            rental_mask = data['匹配文本'].str.contains('|'.join(self.rental_keywords), case=False, na=False)
            final_rental_mask = expense_mask & rental_mask
            data.loc[final_rental_mask, '是否租金支出'] = True
            data.loc[final_rental_mask, '是否重点收支'] = True

        rental_expense_count = data['是否租金支出'].sum()
        self.logger.debug(f"识别出租金支出交易 {rental_expense_count} 笔")
    ...
    def _generate_main_category(self, data: pd.DataFrame):
        ...
        # 车辆收入
        vehicle_income_mask = data['是否车辆收入'] & (data['重点收支类型'] == '')
        data.loc[vehicle_income_mask, '重点收支类型'] = '资产收入'
        data.loc[vehicle_income_mask, '重点收支子类'] = '车辆收入'
        data.loc[vehicle_income_mask, '识别原因'] = '车辆收入关键词匹配'

        # 车辆支出
        vehicle_expense_mask = data['是否车辆支出'] & (data['重点收支类型'] == '')
        data.loc[vehicle_expense_mask, '重点收支类型'] = '资产支出'
        data.loc[vehicle_expense_mask, '重点收支子类'] = '车辆支出'
        data.loc[vehicle_expense_mask, '识别原因'] = '车辆支出关键词匹配'

        # 房产支出
        property_expense_mask = data['是否房产支出'] & (data['重点收支类型'] == '')
        data.loc[property_expense_mask, '重点收支类型'] = '资产支出'
        data.loc[property_expense_mask, '重点收支子类'] = '房产支出'
        data.loc[property_expense_mask, '识别原因'] = '房产支出关键词匹配'

        # 证券收入
        securities_income_mask = data['是否证券收入'] & (data['重点收支类型'] == '')
        data.loc[securities_income_mask, '重点收支类型'] = '资产收入'
        data.loc[securities_income_mask, '重点收支子类'] = '证券收入'
        data.loc[securities_income_mask, '识别原因'] = '证券收入关键词匹配'

        # 证券支出
        securities_expense_mask = data['是否证券支出'] & (data['重点收支类型'] == '')
        data.loc[securities_expense_mask, '重点收支类型'] = '资产支出'
        data.loc[securities_expense_mask, '重点收支子类'] = '证券支出'
        data.loc[securities_expense_mask, '识别原因'] = '证券支出关键词匹配'

        # 租金支出
        rental_expense_mask = data['是否租金支出'] & (data['重点收支类型'] == '')
        data.loc[rental_expense_mask, '重点收支类型'] = '资产支出'
        data.loc[rental_expense_mask, '重点收支子类'] = '租金支出'
        data.loc[rental_expense_mask, '识别原因'] = '租金支出关键词匹配'
    ...
    def generate_statistics(self, data: pd.DataFrame, name_column: str, 
        ...
        for person_name in key_data[name_column].unique():
            ...
            for sub_type, column_name in asset_income_types:
            ...
            total_asset_expense_amount = 0
            total_asset_expense_count = 0

            for sub_type, column_name in asset_expense_types:
                sub_data = person_data[person_data[column_name]]
                stats[f'{sub_type}支出次数'] = len(sub_data)
                stats[f'{sub_type}支出金额'] = abs(sub_data[amount_column].sum()) if not sub_data.empty else 0
                total_asset_expense_amount += stats[f'{sub_type}支出金额']
                total_asset_expense_count += stats[f'{sub_type}支出次数']

            stats['资产支出总额'] = total_asset_expense_amount
            stats['资产支出次数'] = total_asset_expense_count
